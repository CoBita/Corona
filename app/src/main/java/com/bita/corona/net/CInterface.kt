...
import com.bita.corona.repo.data.BaseData
import com.bita.corona.repo.data.Body
...
class CoronaResponseInterceptor : Interceptor {
    ...
    private fun checkSuccess(response: Response): Response {
        ...
        if (header.code != ErrorType.정상.code) {
            throw CoronaException(header.code, header.msg)
        }

        val items = responseToJson.response.body.items
        val result = if (items is String) {
            // 공통 Items이 String 일 때 오류처리
            val convertItemsToObject =
                    Body(Any(), responseToJson.response.body.numOfRows, responseToJson.response.body.pageNo, responseToJson.response.body.totalCount)
...
class CoronaRequestInterceptor(private val serviceKey: String, private val _type: String) : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        val chainRequest = chain.request()
        val httpUrl = chainRequest.url.newBuilder().apply {
            addEncodedQueryParameter(SERVICE_KEY, Const.SERVICE_KEY)
